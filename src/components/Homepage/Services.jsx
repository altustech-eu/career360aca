...
export default function GermanyPathwaysUIKit() {
  ...
  const scroll = (direction) => {
    if (scrollRef.current) {
      ...
      // Scroll by one card width (approx 20% of container)
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 border-b border-slate-200">
          {tabs.map((tab, i) => (
            ...
            >
              ...
              <span className={`font-semibold text-[11px] uppercase tracking-tight ${activeTab === i ? 'text-slate-900' : 'text-slate-500'}`}>{tab.name}</span>
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <aside className="w-full lg:w-[300px] border-r border-slate-200 bg-[#fbfcfd] flex flex-col h-full">
            <div className="p-6 flex flex-col gap-6">
              ...
              <div className="bg-white border border-slate-200 p-5 shadow-xs">
                ...
                <button className="w-full py-3 border border-slate-200 font-semibold flex items-center justify-center gap-2 transition-all uppercase text-[9px] tracking-widest text-blue-700 hover:bg-slate-900 hover:text-white">
          ...
          <main className="flex-grow p-6 lg:p-8 overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-3">
                ...
                {tabs[activeTab].filters.map((t) => (
                  <button 
                    ...
                    className={`px-4 py-1.5 text-[10px] font-semibold uppercase tracking-widest transition-all whitespace-nowrap ${
                      activeSubFilter === t ? `${tabs[activeTab].activeBg} text-white shadow-sm` : 'text-slate-500 hover:bg-slate-50 border border-slate-200'
                    }`}
                  >
                    {t}
                  </button>
              ...
              {/* SLIDER CONTROLS */}
              <div className="flex gap-2 ml-4 text-cyan-500">
            ...
            >
              {filteredCards.length > 0 ? filteredCards.map((card) => (
                <div key={card.id} className="bg-white border border-slate-200 flex-none w-[calc(20%-13px)] min-w-[240px] group flex flex-col shadow-xs hover:border-slate-400 transition-colors h-[480px]">
                  ...
                  <div className="p-4 flex-grow flex flex-col justify-between">
                    ...
                    <div>
                        ...
                        <button className={`w-full text-center text-[9px] font-semibold uppercase tracking-widest hover:underline flex items-center justify-center gap-1.5 py-2 border-t border-slate-50 mt-1 ${tabs[activeTab].color}`}>
                ...
                <div className="w-full flex items-center justify-center text-slate-400 text-[10px] font-semibold uppercase tracking-widest py-12 border border-dashed border-slate-100">
